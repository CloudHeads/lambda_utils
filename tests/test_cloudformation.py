...
def stack_arn(region, account_id, stack_name):
    'arn:aws:cloudformation:{region}:{account_id}:stack/{stack_name}'.format(region=region, account_id=account_id, stack_name=stack_name)
        ...
        return {
            'Records': [{
                ...
                'Sns': {
                    ...
                    '1',
                    'Timestamp':
                    '2017-02-10T14:24:24.208Z',
                    'Signature':
                    '',
                    'SigningCertUrl':
                    'https://sns.eu-west-1.amazonaws.com/SimpleNotificationService-b95095beb82e8f6a046b3aafc7f4149a.pem',
                    'MessageId':
                    '11fe9630-84ae-5327-b49b-c7274bf66063',
                    'Message':
                    json.dumps(custom_resource_event),
                    'MessageAttributes': {},
                    'Type':
                    'Notification',
                    'UnsubscribeUrl':
                    'https://sns.eu-west-1.amazonaws.com/?Action=Unsubscribe&SubscriptionArn=arn:aws:sns:eu-west-1:211099707286:triggerInstanceHealthCheck:803d4d60-e718-44f5-9b5d-9bfe99e4e032',
                    'TopicArn':
                    'arn:aws:sns:eu-west-1:211099707286:triggerInstanceHealthCheck',
                    'Subject':
                    'AWS CloudFormation custom resource request'
                }
            }]
        }
...
def test_function_response(send_signal_mock, event, custom_resource_event):
    @Cloudformation()
...
@patch.object(module, 'send_signal')
def test_exception_raised(send_signal_mock, event, custom_resource_event, random_string):
    @Cloudformation()
    def function(event, context):
